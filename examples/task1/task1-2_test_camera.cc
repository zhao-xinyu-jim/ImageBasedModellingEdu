...
public:
    ...
    math::Vec2d projection(math::Vec3d const & p3d){
        ...
        double x_undistort, y_undistort;
        double r_2;

        //世界坐标系转到了相机坐标系
        x_c = R_[0]*p3d[0] + R_[1]*p3d[1] + R_[2]*p3d[2] + t_[0];
        y_c = R_[3]*p3d[0] + R_[4]*p3d[1] + R_[5]*p3d[2] + t_[1];
        z_c = R_[6]*p3d[0] + R_[7]*p3d[1] + R_[8]*p3d[2] + t_[2];
        //归一化像平面
        x_normal = x_c/z_c;
        y_normal = y_c/z_c;
        //径向畸变
        r_2 = x_normal*x_normal + y_normal*y_normal;
        x_undistort = (1 + dist_[0]*r_2 + dist_[1]*r_2*r_2)*x_normal;
        y_undistort = (1 + dist_[0]*r_2 + dist_[1]*r_2*r_2)*y_normal;
        //转到图像坐标
        p[0] = f_*x_undistort + c_[0];
        p[1] = f_*y_undistort + c_[1];
