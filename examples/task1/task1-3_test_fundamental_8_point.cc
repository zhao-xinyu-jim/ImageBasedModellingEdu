...
FundamentalMatrix fundamental_8_point(math::Matrix<double, 3, 8> const &points1, math::Matrix<double, 3, 8> const &points2)
{
    ...
    FundamentalMatrix F;
    const int cols = points2.cols;
    math::Matrix<double, cols, 9> A;
    std::cout << "A:" << std::endl;
    for (int i = 0; i < cols; i++)
    {
        math::Vec3d x1 = points1.col(i);
        math::Vec3d x2 = points2.col(i);
        double x[9] = {x1[0] * x2[0], x1[1] * x2[0], x2[0], x1[0] * x2[1], x1[1] * x2[1], x2[1], x1[0], x1[1], 1};
        for (int j = 0; j < 9; j++)
            A(i, j) = x[j];
    ...
    std::cout << "Fundamental matrix before singularity constraint is:" << std::endl;
    std::cout << F << std::endl;
