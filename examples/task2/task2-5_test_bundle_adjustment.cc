...
sfm::Correspondences2D2D sift_feature_matching(sfm::FeatureSet &feat1, sfm::FeatureSet &feat2)
{
    ...
    features::Matching::twoway_match(matching_opts, aligned_descrs1.data()->begin(), feat1.sift_descriptors.size(), aligned_descrs2.data()->begin(), feat2.sift_descriptors.size(), &matching_result);
    // 去除不一致的匹配对
...
bool calc_cam_poses(sfm::Correspondences2D2D const &matches, sfm::CameraPose *pose1, sfm::CameraPose *pose2, float f1, float f2)
{
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 3; i++)
        out << cams[0].translation[i] << " ";
    for (int i = 0; i < 9; i++)
        out << cams[0].rotation[i] << " ";
    out << std::endl;
    ...
    for (int i = 0; i < 3; i++)
        out << cams[1].translation[i] << " ";
    for (int i = 0; i < 9; i++)
        out << cams[1].rotation[i] << " ";
    out << std::endl;
